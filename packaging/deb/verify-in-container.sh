...
for exe in /opt/cc-computer-use/vendor/bin/*; do
  if ldd "$exe" 2>/dev/null | grep -q "not found"; then
    ...
    ldd "$exe" 2>/dev/null | grep "not found" | sed 's/^/      /'
  fi
done
...
su - tester -c "cc-computer-use-setup" 2>&1 | sed 's/^/  /'
echo "  --- claude 实际被这样调用 ---"; sed 's/^/    /' /tmp/claude-calls.log
...
step "[7] 体检（无显示环境）"
/usr/bin/cc-computer-use-doctor 2>&1 | sed 's/^/  /' || true
ok "doctor 能跑（无 DISPLAY 时跳过服务自检属正常）"
