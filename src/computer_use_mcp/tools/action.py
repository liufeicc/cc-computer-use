...
def register(mcp: Any, coord: Coordinator) -> None:
    ...
    @mcp.tool(
        ...
        description=(
            ...
            "⭐ 使用判据：**凡是下一步不依赖上一步结果的连续动作，一律一次提交**；"
            "每多一次单独调用，就多一个「模型思考 + 读结果」的来回（实测每轮 30~70 秒，"
            "而工具本身只要零点几秒）。只有**需要看结果做分支判断**时才拆成多次调用。"
            ...
            "sleep{seconds} / list_windows{} / screenshot{region?,max_side?}。"
            "**把 screenshot 放在最后一步**，可以在同一次调用里拿到「这一串做完之后长什么样」，"
            "省掉单独截图的整个来回（确认类截图占实测截图的三分之二）。"
