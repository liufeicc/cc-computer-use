...
- **`type_text` 非 ASCII 必须走剪贴板**（xclip + ctrl+v，`inject._type_via_clipboard`）：`xdotool type` 对中文会**临时改写全局键盘映射**（XChangeKeyboardMapping），输入期间用户物理键盘整体失灵（鼠标正常）、写完才恢复，且与 fcitx 输入法争抢 XKB 状态。纯 ASCII 才走 xdotool type。
